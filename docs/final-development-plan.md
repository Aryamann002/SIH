...
Last updated: **2026-09-24 (Asia/Calcutta)**. Planning baseline: `396f36c9d4f3e48fba4ebaa3aad7a670daf28af4`, with pre-existing local configuration/runtime changes.
...
- **Current stage:** B01-B05, D01, S01-S03, V01 and V04 have passing receipts. V04 bounded unauthenticated session/pre-auth stream admission and reviewed local security boundaries; the `v04` image passed backend and fake-device browser action checks. D02 is blocked on rights/consent and independent four-language sources. Jev shadow works in mocked/no-key checks; a live Jev run is unavailable.
- **Next task:** V03 physical-device/first-alert-distribution checks and the remaining V02 database-failure sweep while seeking permitted, leakage-checked data for representative Stage 0 validation. R01's `v03` offline archives are now stale after V04 and need rebuilding, plus pristine-host Compose rehearsal, complete docs bundle and final freeze.
...
| M01 | Days 9-10 / evaluation | IN_PROGRESS | Extend existing evaluator for language, duration, channel, replay, held-out generator and partial-manipulation slices; log raw/scorable/rejected counts | EER/AUC/precision/F1/coverage/no-alert and metadata slices added; rich manifest validated. Duration/replay/partial-spoof samples and first-alert distribution remain unavailable |
...
| V02 | Days 14-16 / tester | IN_PROGRESS | Prove expiry/3-attempt lockout, reused/wrong-action tokens, foreign sessions, concurrent completion, malformed/oversized audio, stream loss, model/DB/audit failure and restart safety | [S03/V01 receipt](validation/s03-2026-09-24.md): final-image API 20/20, including new red/green same-session WAV overload regression; the original upload route could replace the live generation before capacity rejection. [V03 checkpoint](validation/v03-2026-09-24.md): final-image 5/5 hard-restart recovery checks. DB failure sweep and physical workflow remain. |
| V03 | Days 14-16 / performance | IN_PROGRESS | Measure cold/warm start, WAV latency, live first-alert and evidence age, 1/2/3-session stress and long-run stability on presentation hardware | [V03 checkpoint](validation/v03-2026-09-24.md): final image 12 warmed 10-second WAVs median 5.19 s/p95 5.26 s; 2,999 frames/600.2 s with 11.83 ms median/400.01 ms p95 response and stable sampled 784 MiB backend RSS; warm restart-to-ready 13.31 s. One demo synthetic clip first HIGH at 2.4 audio seconds. Physical mic, full cold start, first-alert distribution and sustained multiuser work remain. |
...
| R01 | Day 17 / integration | IN_PROGRESS | Package exact tested dependencies/images/models, safe Docker context and offline startup; verify new clone/source export plus preserved-db restart | [R01 checkpoint](validation/r01-packaging-2026-09-24.md) and [offline package](validation/offline-package-2026-09-24.md): pinned base/28 packages, `v03` source/backend archives matched to their tested image, 20/20 no-egress checks, 5/5 hard restart on preserved isolated test DB. V04 source changes make those archives stale for the current tree. Needs fresh package, pristine-host Compose launch, complete docs bundle and final-source freeze before DONE. |

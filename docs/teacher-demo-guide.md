...
This is a rehearsal guide for the current local prototype, not a claim that the final October 3 presentation build is frozen. Open `http://127.0.0.1:8000/` for the operator and `/verify` for the local verifier. The [refreshed slides](teacher-presentation.pdf) and [handbook](VigilVoice-Build-and-Launch-Guide.pdf) describe the September 25 source and must be rechecked after the final freeze. The [silent walkthrough](vigilvoice-teacher-walkthrough.mp4) predates continuous streaming; do not present it as current-build proof.
...
4. **HIGH retention:** Explain that later LOW audio cannot revive an already blocked action. The [current-source browser/action receipt](validation/r02-action-browser-2026-09-25/browser-checks.json) includes a genuine → synthetic HIGH → genuine transition and a direct completion denial after later LOW; its [HIGH/BLOCKED screenshot](validation/r02-action-browser-2026-09-25/teacher-demo.png) is a controlled fake-device case. Show that evidence if a physical source-switch rehearsal is unavailable; do not claim a real-microphone transition test.
...
| Database timeout / 503 during an action | Do not assume the transaction rolled back. After readiness recovers, refresh the known action ID and audit before retrying. If no action ID was returned, pause the demo rather than treating an unknown outcome as approval. |
| Backend/model unavailable | Check `docker compose ps` and `/readyz`. The refreshed slides explain the current design; the old silent video is only a labeled historical fallback and does not prove current streaming or HIGH/BLOCKED. |
...
Pending: physical-microphone and disconnected-internet final-build rehearsal, representative four-language and phone/replay/partial-spoof evaluation, defensible model/policy freeze, live Jev shadow evaluation and current narrated backup video. The deck and handbook are refreshed but still need a final-source re-render. A trained SSL head is conditional, not a requirement. Real SMS/payment integration and production hosting are out of scope.

...
Read [docs/final-development-plan.md](docs/final-development-plan.md) at every
session start, then inspect `git status --short`. It is the authoritative plan,
task ledger and handoff for VigilVoice's October 3, 2026 presentation.
Update task status and evidence after material progress; append a dated session
handoff with blockers and the next action before ending. Never mark DONE without
the task's acceptance checks; reopen tasks if changes invalidate their evidence.
The application is Python/FastAPI with PostgreSQL, ONNX Runtime and native browser
JavaScript; use the plan/runbook commands, not the generated npm build examples below.
